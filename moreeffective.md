Markdown  
\# Phân Tích Sâu: 35 Quy Tắc "More Effective C++" (Scott Meyers) \- Phiên bản Hiện Đại Hóa

# Fix: "name followed by '::' must be a class or namespace name"

Tài liệu này giải thích nguyên nhân gây ra lỗi biên dịch "name followed by '::' must be a class or namespace name", đưa ra cách khắc phục và ví dụ mã nguồn đã sửa. Nội dung tập trung vào trường hợp liên quan tới `std::variant` và `std::format` (như trong ví dụ trước), cùng các lưu ý thường gặp.

---

## Tóm tắt

- Nguyên nhân thường gặp: bạn đang dùng `T::something` (dấu `::`) với tên `T` mà trình biên dịch hiểu là giá trị (object) hoặc không phải là kiểu/namespace. Ví dụ: `w1::getCount()` — `w1` là object, không phải kiểu.
- Trường hợp đặc biệt với `std::variant` + `std::format`: nếu bạn gọi `std::get<T>(v)` với `T` không phải là một trong các alternative của `v`, biên dịch viên sẽ sinh ra các thông báo lỗi có vẻ lạ, thậm chí có thể xuất hiện lỗi liên quan đến `::` do cách các template/traits được hiện thực hoá.

## Nguyên nhân cụ thể (ví dụ liên quan tới `std::variant`)

- `std::get<T>(variant)` yêu cầu `T` phải nằm trong danh sách các loại của `variant`.
- Nếu bạn gán một kiểu khác (ví dụ `double`) vào `std::variant<int, std::string>` rồi gọi `std::get<double>(v)`, thì `std::get<double>` không là một overload hợp lệ cho `variant<int,std::string>`; khi template machinery cố gắng chọn overload/instantiate, các thông báo lỗi nội bộ có thể dẫn đến chỗ thông báo như "name followed by '::' must be a class or namespace name".

Ngắn gọn: luôn kiểm tra xem kiểu bạn truyền cho `std::get<T>` có nằm trong `std::variant<...>` hay không; nếu không chắc, dùng `std::get_if` hoặc `std::visit`.

---

## Các cách sửa và đề xuất

1. Thêm kiểu bị thiếu vào `std::variant` nếu bạn thực sự muốn lưu kiểu đó.

2. Hoặc giữ `variant` như cũ và gán một giá trị có kiểu nằm trong danh sách alternatives (chẳng hạn cast hoặc convert trước khi gán).

3. Dùng truy cập an toàn:

   - `std::get_if<T>(&v)` — trả về con trỏ nếu chứa `T`, hoặc `nullptr` nếu không.
   - `std::visit(visitor, v)` — xử lý mọi trường hợp an toàn.

4. Cẩn trọng với `std::format` (và kiểm tra format-string): `std::format` có kiểm tra tại compile-time (với `std::format_string`) nên mismatched types có thể gây lỗi compile-time khá phức tạp. Luôn truyền kiểu đúng vào formatter.

---

## Ví dụ minh họa (sửa lỗi trong đoạn code trước)

Đoạn gốc gây lỗi (tóm tắt):

```cpp
// giả sử có: std::variant<int, std::string> var;
// và sau đó:
var = std::numbers::pi; // double
std::cout << std::format("Gia tri: {}\n", std::get<double>(var));
```

Sửa an toàn bằng cách thêm `double` vào `variant`:
...
int main()
{
    std::variant<int, double, std::string> var; // thêm double
    var = std::numbers::pi;

    std::cout << std::format("Gia tri cua var {:.3f}\n", std::get<double>(var));

    std::string str {"Con ca nho"};
    var = str;
    std::cout << std::format("Gia tri cua var {}\n", std::get<std::string>(var));

    return 0;
}
```

Hoặc dùng `std::visit` để tránh phải biết trước kiểu:
...
Lưu ý: dùng `std::format` với `auto` trong `std::visit` vẫn cần đảm bảo `operator<<`/formatter` cho các loại được hỗ trợ, hoặc tùy biến `formatter` cho kiểu của bạn.

---

## Các lỗi thường gặp và cách đọc chúng

- "name followed by '::' must be a class or namespace name" thường xuất hiện khi bạn viết `object::member` thay vì `Type::member` hoặc `namespace::name`.
- Kiểm tra chỗ dùng `::` — `::` chỉ hợp lệ sau một tên kiểu hoặc namespace (ví dụ `Widget::count`, `std::string`).

---

## Checklist sửa lỗi nhanh

- [ ] Kiểm tra nơi báo lỗi: có dùng `::` sau một object không? Nếu có, đổi `obj::member` thành `obj.member`.
- [ ] Nếu lỗi liên quan templates/variant/format: kiểm tra các kiểu truyền vào `std::get<T>` và format-argument cặp với format-spec.
- [ ] Nếu dùng macro/typedef/template khiến tên xung đột, thử tạm đổi tên hoặc viết ví dụ tối giản để tái tạo lỗi.

---

## Muốn mình giúp gì tiếp theo?

- Mình có thể: cập nhật file nguồn mẫu trong workspace, sửa trực tiếp `moreeffective.md` (đã làm), hoặc biên dịch/kiểm thử ví dụ của bạn.
- Nếu bạn dán đoạn mã gây lỗi thực tế (toàn bộ), mình sẽ sửa và chạy thử, rồi trả về bản sửa chi tiết.

---

**File:** `c:\Users\Admin\Desktop\OOP\ScottMeyers\moreeffectivec++\moreeffective.md`


Tài liệu này phân tích chi tiết 35 quy tắc (Items) trong cuốn sách kinh điển "More Effective C++" của Scott Meyers. Mỗi Item được phân tích theo 4 mục: Tóm tắt, Lý do, Ví dụ Code (Trước & Sau), và Bối cảnh/Ngoại lệ, đồng thời được bổ sung các "Lưu ý C++ Hiện Đại" để cập nhật kiến thức cho C++11/17/20.
